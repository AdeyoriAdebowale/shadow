...
var inviteLink = []; // Array of invite links.
var inviteCount = 0; // Logs how many servers the account has joined.
var serverCount = undefined; // amount of servers, in case the account is already in a server.
var serverID = []; // server ID's scraped from discordserver.me.
var userServersID = []; // all server ID's account is in.
var index;
...
(async () => {
    try {
        console.log(chalk.magenta("Attemping to start Shadow."));
        try {
            await client.login(config.token);
            serverCount = client.guilds.size // stores amount of servers into serverCount variable
            if (serverCount == 100) {
        ...
        var serversID = []; // array of unfiltered junk
        var count = 1;
        ...
        async function pageLoop() {
            ...
            for (const link of links) {
                if (link.indexOf('/servers/') !== -1) {
                    let linkFilter = await link.split('"')[0].split('/join')[0].split('/servers/tag/')[0].split('https://')[0].split('/servers/top')[0].split('/auth/discord')[0].split('#!')[0];
                    serversID.push(linkFilter); // pushes into serversID array to filter later
                }
            }
            
            var filtered = [...new Set(serversID)]; // puts into set and removes dupes
            ...
            if (result.length <= 125) {
                ...
                guildIds.forEach(function (ids) {
                    let b4Push = ids.split(',')[0]; // epic var name
            ...
            if (result.length >= 125) {
                ...
                if (!userServersID.length == 0) { // removes serverid dupliactes
                    for (var i = 0; i < userServersID.length; i++) {
                ...
                for (let i = 0; i < dupeFilter.length; i++) {
                    ...
                    if (res[1] === "ZnwusVc") {
                        console.log(chalk.red("Couldn't get invite, trying next.")); // shitty throw of link thrown in by discordservers.me
                    } else {
                        inviteLink.push(res[1]);
                        inviteLink.forEach(async function (invites) {
                            inviteCount++;
                            let invite = invites;
                                ...
                                if (err.response.data.code == 40002) {
                                    ...
                                    if (err.response.data.code == 40007) {
                                        ...
                                        if (err.response.data.code == 30001) {
                                            ...
                                            console.log(chalk.red(err.response.data.code));
                                            process.exit(-1)
                                        }
                                    }
                                }
